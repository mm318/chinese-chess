...
//Class:       CCElephant
//Description: Digital representation of an Elephant piece
...
/**
 * Digital representation of an Elephant piece.
 */
public class CCElephant implements CCPieces
{
    /** value of the piece */
     ...
     */
    public CCElephant (CCPieces board[] [], boolean playerOne, int x, int y)
    {
	...
	if (playerOne)
	{
	    icon = new JLabel (new ImageIcon (getClass ().getResource ("images/ElephantRed.png")));
	    PIECE_TYPE = CCPieces.ELEPHANT1;
	}
	else
	{
	    icon = new JLabel (new ImageIcon (getClass ().getResource ("images/ElephantGreen.png")));
	    PIECE_TYPE = CCPieces.ELEPHANT2;
	}
	...
	if (Math.abs (x2 - x1) != 2 || Math.abs (y2 - y1) != 2)
	    return false;
	else if ((isPlayerOne && y2 < 5) || (!isPlayerOne && y2 > 4))
	    return false;
	else if (board [(y1 + y2) / 2] [(x1 + x2) / 2] != null)
	    return false;
